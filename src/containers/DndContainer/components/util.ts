...
export type CardsState = { [key in Columns]: Props[] };
...
export const moveCardToColumn = ({
  ...
  const newCards = {
    ...cards,
    [card.previousColumn]: cards[card.previousColumn].filter((card) => card.id !== cardId),
    [column]: [...cards[column].slice(0, index), card.card, ...cards[column].slice(index)],
  };
